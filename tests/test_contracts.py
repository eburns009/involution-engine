...
def test_health_endpoint_contract() -> None:
    ...
    # Standard structure for both success and error
    assert "data" in data
    assert "meta" in data
    assert isinstance(data["data"], dict)
    assert isinstance(data["meta"], dict)

    # Meta field contract (same for both success and error)
    meta = data["meta"]
    required_meta_fields = {
        "service_version", "spice_version", "kernel_set_tag",
        "ecliptic_frame", "coordinate_system", "obliquity_model",
        "aberration_correction", "request_id", "timestamp"
    }
    assert set(meta.keys()) == required_meta_fields
    assert isinstance(meta["service_version"], str)
    assert isinstance(meta["spice_version"], str)
    assert isinstance(meta["kernel_set_tag"], str)
    assert meta["ecliptic_frame"] == "ECLIPDATE"
    assert meta["coordinate_system"] == "ecliptic_of_date"
    assert meta["obliquity_model"] == "IAU1980-mean"
    assert meta["aberration_correction"] == "LT+S"
    assert isinstance(meta["request_id"], str)
    assert isinstance(meta["timestamp"], (int, float))
    ...
    if data["status"] == "healthy":
        ...
        required_data_fields = {"kernels_loaded", "earth_radii_km"}
        assert set(health_data.keys()) == required_data_fields
        assert isinstance(health_data["kernels_loaded"], int)
        assert isinstance(health_data["earth_radii_km"], list)
        assert len(health_data["earth_radii_km"]) == 3
...
def test_info_endpoint_contract() -> None:
    """Contract test: /info endpoint format"""
    client = get_client()
    r = client.get("/info")
    ...
    if data["status"] == "ok":
        # Success info contract
        info_data = data["data"]
        required_data_fields = {"kernels", "kernel_count"}
        assert set(info_data.keys()) == required_data_fields
        assert isinstance(info_data["kernels"], list)
        assert isinstance(info_data["kernel_count"], int)
        assert info_data["kernel_count"] == len(info_data["kernels"])

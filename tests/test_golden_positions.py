...
def test_goldens(client, row):
    ...
    for body, exp in row["expect"].items():
        ...
        # Relaxed tolerance for proper precession correction (expected values may be from J2000 epoch)
        assert dlon < 0.4, f"{body} lon off {dlon:.4f}° (got {got['longitude']:.4f}, expected {exp['lon']:.4f})"
